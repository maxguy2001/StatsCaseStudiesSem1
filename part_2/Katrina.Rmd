...
df_remove_na_columns <- df[,!(names(df) %in% c( "coupleid", 
...
# adding all the euro columns (1-10) but taking NA as 0? so they add together and the row doesnt have all NA if only one NA present in 1-10
# df_remove_na_columns[df_remove_na_columns$euro1 == NA] <- 0
  
  
colSums(is.na(df_remove_na_columns))

ncol(df_remove_na_columns %>% drop_na())
```
